namespace Log_Parser_App.Services
{
    ...
    {
        private static readonly Regex _commonLogFormat = MyRegex10();

        private static readonly Regex _standardLogFormat = MyRegex9();

        private static readonly Regex _configUpdateLogFormat = MyRegex11();
        ...
        public async Task<IEnumerable<LogEntry>> ParseLogFileAsync(string filePath) {
            ...
            logger.LogInformation("Parsed {Count} log entries from {LineCount} lines in {ElapsedMs} ms", logEntries.Count, lineCount, sw.ElapsedMilliseconds);
            return logEntries;
        }
        ...
        public Task<IEnumerable<LogEntry>> ExecuteQueryAsync(IEnumerable<LogEntry> logEntries, string query) {
            ...
            try {
                ...
                if (!hasWhereCondition && !hasFromCondition && query.Contains('=')) {
                    try {
                        ...
                        if (parts.Length == 2) {
                            ...
                            string fieldValue = parts[1].Trim().Trim('"', '\'');

                            if (fieldName.Equals("Message", StringComparison.OrdinalIgnoreCase) || fieldName.Equals("Message", StringComparison.OrdinalIgnoreCase)) {
                            ...
                            } else if (fieldName.Equals("Level", StringComparison.OrdinalIgnoreCase) || fieldName.Equals("Level", StringComparison.OrdinalIgnoreCase)) {
                            ...
                            } else if (fieldName.Equals("Source", StringComparison.OrdinalIgnoreCase) || fieldName.Equals("Source", StringComparison.OrdinalIgnoreCase)) {
...

                {
                    ...
                    int whereIndex = normalizedQuery.IndexOf(conditionKeyword, StringComparison.Ordinal) + conditionKeyword.Length;
                    string wherePart = query.Substring(whereIndex).Trim();
                    string normalizedWherePart = normalizedQuery[whereIndex..].Trim();

                    if ((normalizedWherePart.Contains("LEVEL", StringComparison.OrdinalIgnoreCase) || normalizedWherePart.Contains("LEVEL", StringComparison.OrdinalIgnoreCase)) &&
                        normalizedWherePart.Contains('=')) {
                        string level = ExtractValueFromCondition(normalizedWherePart, "LEVEL");
                        if (!string.IsNullOrEmpty(level))
                            logEntries = enumerable.Where(e => e.Level.Equals(level, StringComparison.OrdinalIgnoreCase));
                    }

                    if ((normalizedWherePart.Contains("SOURCE", StringComparison.OrdinalIgnoreCase) || normalizedWherePart.Contains("SOURCE", StringComparison.OrdinalIgnoreCase)) &&
                        normalizedWherePart.Contains('=')) {
                        string source = ExtractValueFromCondition(normalizedWherePart, "SOURCE") ?? ExtractValueFromCondition(normalizedWherePart, "SOURCE");
                        if (!string.IsNullOrEmpty(source))
                            logEntries = enumerable.Where(e => e.Source.Equals(source, StringComparison.OrdinalIgnoreCase));
                    }

                    if (normalizedWherePart.Contains("MESSAGE", StringComparison.OrdinalIgnoreCase) || normalizedWherePart.Contains("MESSAGE", StringComparison.OrdinalIgnoreCase)) {
                        if (normalizedWherePart.Contains("LIKE")) {
                            var messageText = ExtractValueFromLikeCondition(normalizedWherePart, "MESSAGE") ?? ExtractValueFromLikeCondition(normalizedWherePart, "MESSAGE");
                            ...
                            string messageText = ExtractValueFromCondition(normalizedWherePart, "MESSAGE") ?? ExtractValueFromCondition(normalizedWherePart, "MESSAGE");
                            if (!string.IsNullOrEmpty(messageText)) {
...

                    {
                        ...
                        {
                            string dateString = ExtractValueFromTimeCondition(normalizedWherePart, "TIMESTAMP", "<");
                            if (DateTime.TryParse(dateString, out var date)) {
                                logEntries = enumerable.Where(e => e.Timestamp < date);
                            }
                        }
        ...
        public async Task<string> DetectLogFormatAsync(string filePath) {
            ...
            // Check log formats
            foreach (string line in sample.Where(line => !string.IsNullOrWhiteSpace(line))) {
                if (_configUpdateLogFormat.IsMatch(line)) {
                    logger.LogInformation("Determined log format: ConfigUpdate by content");
                    return "ConfigUpdate";
                }

                if (_standardLogFormat.IsMatch(line)) {
                    logger.LogInformation("Determined log format: Standard");
                    return "Standard";
                }

                if (_commonLogFormat.IsMatch(line)) {
            ...
            try {
                ...
                switch (format.ToLowerInvariant()) {
                    ...
                    case "configuration":
                        ...
                        break;
                    default:
                        entry = ((ParseStandardLogFormat(line, filePath, lineCount) ?? ParseCommonLogFormat(line, filePath, lineCount)) ?? ParseCsvLogFormat(line, filePath, lineCount)) ??
                            ParseSimpleFormat(line, filePath, lineCount);
                        break;
                }
                ...
                try {
                    ...
                    var resultMatch = MyRegex12().Match(messageToCheck);
                    if (resultMatch is { Success: true, Groups.Count: > 1 }) {
                    ...
                    if (entry.Level != "ERROR" && entry.Level != "WARNING" && entry.Level != "INFO") {
        ...
        private LogEntry? ParseStandardLogFormat(string line, string filePath, int lineCount) {
            var match = _standardLogFormat.Match(line);
            if (!match.Success)
                return null;

            if (!DateTime.TryParseExact(match.Groups[1].Value, _dateFormats, null, System.Globalization.DateTimeStyles.None, out var timestamp)) {
                timestamp = DateTime.Now;
            }
            ...
            } else if (MyRegex13().IsMatch(message)) {
                level = "WARNING";
        ...
        private LogEntry? ParseCommonLogFormat(string line, string filePath, int lineCount) {
            var match = _commonLogFormat.Match(line);
            if (!match.Success)
                return null;

            if (!DateTime.TryParseExact(match.Groups[2].Value, _dateFormats, null, System.Globalization.DateTimeStyles.None, out var timestamp)) {
                timestamp = DateTime.Now;
            }
        ...
        private LogEntry? ParseCsvLogFormat(string line, string filePath, int lineCount) {
            ...
            if (!DateTime.TryParseExact(parts[0], _dateFormats, null, System.Globalization.DateTimeStyles.None, out var timestamp)) {
                timestamp = DateTime.Now;
            }
        ...
        private string DetermineLogLevel(string originalLevel, string message) {
            ...
            if (message.Equals("Error", StringComparison.Ordinal) || message.Equals("ERROR", StringComparison.Ordinal) || message.Equals("error", StringComparison.Ordinal)) {
                logger.LogDebug("Exact match of message with word Error: '{0}'", message);
                return "ERROR";
            }

            if (message.Equals("Warning", StringComparison.Ordinal) || message.Equals("WARNING", StringComparison.Ordinal) || message.Equals("warning", StringComparison.Ordinal)) {
        ...
        public Task<IEnumerable<LogEntry>> FilterErrorsAsync(IEnumerable<LogEntry> logEntries) {
            ...
            var filteredEntries = logEntries.Where(e => e.Level.Trim().ToUpperInvariant() == "ERROR").ToList();
        ...
        private LogEntry? ParseSimpleFormat(string line, string filePath, int lineCount) {
            try {
                ...
                if (line.Contains("System.ApplicationException") || line.Contains("Exception:") || Regex.IsMatch(line, @"\bError\b|\bERROR\b|\berror\b")) {
                    level = "ERROR";
                    logger.LogDebug("Found error indicator in message: {Message}", line[..Math.Min(50, line.Length)]);
                ...
                logger.LogWarning(ex, "Error parsing simple format: {Line}", line.Substring(0, Math.Min(50, line.Length)));
                return null;
            }
        }
        ...
        private LogEntry? ParseConfigUpdateFormat(string line, string filePath, int lineCount) {
            try {
                ...
                logger.LogDebug("ConfigUpdate log: IsError={IsError}, IsWarning={IsWarning}, Source={Source}", isError, isWarning, source);
                ...
                logger.LogWarning(ex, "Error parsing ConfigUpdate format: {Line}", line.Substring(0, Math.Min(line.Length, 50)));
                return null;
            }
        }
        ...
        public async Task<IEnumerable<PackageLogEntry>> ParsePackageLogFileAsync(string filePath) {
            ...
            try {
                while (await reader.ReadLineAsync() is { } line) {
                    ...
                    if (line.Contains("Installing package") || line.Contains("Updating package") || line.Contains("Removing package") || line.Contains("Restoring package")) {
                        if (isCollectingPackageInfo && !string.IsNullOrEmpty(currentPackage.PackageId)) {
                            packageLogEntries.Add(currentPackage);
                        }
                        ...
                        if (line.Contains("Installing"))
                            currentPackage.Operation = "install";
                        else if (line.Contains("Updating"))
                            currentPackage.Operation = "update";
                        else if (line.Contains("Removing"))
                            currentPackage.Operation = "remove";
                        else if (line.Contains("Restoring"))
                            currentPackage.Operation = "restore";
                        ...
                        switch (isCollectingPackageInfo) {
                            case true when line.Contains("Dependencies:"):
                                currentPackage.Dependencies = line.Replace("Dependencies:", "").Trim();
                                break;
                            case true when (line.Contains("successfully") || line.Contains("failed") || line.Contains("error")): {
            ...
            logger.LogInformation("Parsed {Count} package log entries from {LineCount} lines in {ElapsedMs} ms", packageLogEntries.Count, lineCount, sw.ElapsedMilliseconds);
        ...
        [GeneratedRegex(@"result:\s*'([^']+)'", RegexOptions.IgnoreCase, "en-US")]
