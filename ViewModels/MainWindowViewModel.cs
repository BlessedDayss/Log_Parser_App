namespace Log_Parser_App.ViewModels
{
    ...
    public partial class MainWindowViewModel : ViewModelBase
    {
        ...
        private async Task ApplyFilterCriterionCommand(FilterCriterion? criterion) {
            ...
            if (string.IsNullOrWhiteSpace(criterion.SelectedField) || string.IsNullOrWhiteSpace(criterion.SelectedOperator) || criterion.Value == null) {
            ...
            try {
                ...
                await Task.Run(() => {
                    ...
                    foreach (var criterion in FilterCriteria) {
                        if (string.IsNullOrWhiteSpace(criterion.SelectedField) || string.IsNullOrWhiteSpace(criterion.SelectedOperator) || criterion.Value == null) // Check for null value
                        {
                            _logger.LogWarning("Skipping incomplete filter criterion: Field='{Field}', Operator='{Operator}', Value='{Value}'",
                                criterion.SelectedField,
                                criterion.SelectedOperator,
                                criterion.Value);
                            continue;
                        }
