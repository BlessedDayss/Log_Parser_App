namespace Log_Parser_App.ViewModels
{
    ...
    public partial class MainViewModel : ViewModelBase
    {
        ...
        public MainViewModel(ILogParserService logParserService, ILogger<MainViewModel> logger, IFileService fileService, IErrorRecommendationService errorRecommendationService) {
        ...
        private void UpdateCharts() {
            try {
                ...
                ErrorTrendSeries = errorTrend.Any(v => v > 0)
                    ? new ISeries[] {
                ...
                // 5. Top Errors Chart
                var topErrors = LogEntries.Where(e => e.Level == "ERROR").GroupBy(e => e.Message).Select(g => new { Message = g.Key, Count = g.Count() }).OrderByDescending(x => x.Count)
                    .Take(10).ToList();
                ...
                var sourcesData = LogEntries.GroupBy(e => e.Source).Where(g => !string.IsNullOrEmpty(g.Key)) // Filter out empty sources
                    .Select(g => new { Source = g.Key, Count = g.Count() }).OrderByDescending(x => x.Count).Take(8).ToList();
