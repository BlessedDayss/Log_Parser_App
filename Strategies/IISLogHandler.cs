...
namespace Log_Parser_App.Strategies
{
    ...
    /// Implements Strategy pattern for IIS-specific log operations
    /// </summary>
    public class IISLogHandler : ILogTypeHandler
    {
        ...
        public IISLogHandler(ILogger<IISLogHandler> logger, IIISLogParserService iisLogParserService)
        {
            _logger = logger;
        ...
        /// <summary>
        /// Parse IIS log file using IIS-specific parsing logic
        /// </summary>
        public async Task<IEnumerable<LogEntry>> ParseLogFileAsync(string filePath)
        {
            try
            {
                _logger.LogDebug($"Parsing IIS log file: {filePath}");

                var iisEntries = _iisLogParserService.ParseLogFileAsync(filePath, default);
                var logEntries = new List<LogEntry>();

                await foreach (var iisEntry in iisEntries)
                {
                    var logEntry = ConvertIISToLogEntry(iisEntry);
        ...
        public LogMetadata ExtractMetadata(IEnumerable<LogEntry> entries)
        {
            try
            {
                ...
                var metadata = new LogMetadata
                {
                    ...
                    Sources = ipAddresses,
                    ErrorTypes = statusCodes.Where(kvp => kvp.Key.StartsWith("4") || kvp.Key.StartsWith("5"))
                                           .ToDictionary(kvp => $"HTTP {kvp.Key}", kvp => kvp.Value),
        ...
        private LogEntry ConvertIISToLogEntry(IisLogEntry iisEntry)
        {
            ...
            return new LogEntry
            {
                ...
                Message = message,
                Source = iisEntry.ClientIPAddress,
                RawData = iisEntry.RawLine,
        ...
        {
            var totalRequests = statusCodes.Values.Sum();
            var errorRequests = statusCodes.Where(kvp => kvp.Key.StartsWith("4") || kvp.Key.StartsWith("5")).Sum(kvp => kvp.Value);
        ...
        private Dictionary<string, int> ExtractPopularEndpoints(List<LogEntry> entries)
        {
            ...
            var uriPattern = @"(GET|POST|PUT|DELETE|HEAD|OPTIONS|PATCH) ([^\s?]+)";
        ...
        {
            var validMethods = new[] { "GET", "POST", "PUT", "DELETE", "HEAD", "OPTIONS", "PATCH", "TRACE", "CONNECT" };
